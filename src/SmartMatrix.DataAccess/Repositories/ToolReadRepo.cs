...
{
    public class ToolReadRepo<T, TId> : IToolReadRepo<T, TId> where T : AuditableEntity<TId>
    {
        const double DefaultCommandTimeoutInSeconds = 60;

        private ToolReadDbContext _readDbContext;        
        public IToolReadDbContext ToolReadDb => _readDbContext;        
        public IQueryable<T> Entities => _readDbContext.Set<T>();

        public ToolReadRepo(IToolReadDbContext readDbContext)
        {
            _readDbContext = (ToolReadDbContext)readDbContext;
