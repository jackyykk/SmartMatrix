...
{
    public class AppWriteRepo<T, TId> : IAppWriteRepo<T, TId> where T : AuditableEntity<TId>
    {
        const double DefaultCommandTimeoutInSeconds = 60;

        private readonly AppWriteDbContext _writeDbContext;        
        public IAppWriteDbContext AppWriteDb => _writeDbContext;
        ...
        public AppWriteRepo(IAppReadDbContext readDbContext, IAppWriteDbContext writeDbContext)
        {            
            _writeDbContext = (AppWriteDbContext)writeDbContext;
