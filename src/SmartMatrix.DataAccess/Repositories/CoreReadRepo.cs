...
{
    public class CoreReadRepo<T, TId> : ICoreReadRepo<T, TId> where T : AuditableEntity<TId>
    {
        const double DefaultCommandTimeoutInSeconds = 60;

        private CoreReadDbContext _readDbContext;        
        public ICoreReadDbContext CoreReadDb => _readDbContext;        
        public IQueryable<T> Entities => _readDbContext.Set<T>();

        public CoreReadRepo(ICoreReadDbContext readDbContext)
        {
            _readDbContext = (CoreReadDbContext)readDbContext;
